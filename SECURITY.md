...
- The public Circuit Relay v2 service is intentionally available to pseudonymous peers. A circuit can terminate only at an online peer with a live reservation on this relay; it is not an arbitrary IP proxy. Reservations, circuit duration, circuit bytes, STOP streams and history-sync throughput are all bounded to limit abuse.
