...
| `relay.laozi.art` | Carbon Club | Volunteer Mac bootstrap/relay over WSS | Target production relay name. |
...
| Gname | Domain registrar / current DNS | Registration and, until migration, authoritative DNS for `laozi.art` | Registrar security settings remain user-managed. |
| Cloudflare | DNS and tunnel edge | Planned authoritative DNS for `laozi.art`; active tunnel edge | Nameserver, deletion, token, permission, billing, and production cutover changes require owner approval. |
...
- The dedicated `dsh-carbon-club-relay` tunnel is healthy and its intended
  public hostname is `relay.laozi.art`.
- Carbon Club documentation still publishes
  `carbon-relay.a2a-crucible.xyz` as a temporary compatibility endpoint. This is
  the only permitted cross-project exception and must not be copied into source,
  deploy templates, package metadata, or documentation other than the existing
  compatibility instructions and this boundary record.
...
- `relay.laozi.art`：碳基会所志愿 Mac 中继的目标正式地址。
...
- Vercel：`laozi.art` 网站的托管平台，不是新增域名；迁移 DNS 时必须保留其
  站点解析记录。
- Gname：域名注册商，同时仍是 `laozi.art` 当前权威 DNS。
- Cloudflare：计划接管 `laozi.art` 权威 DNS，并承载独立碳基会所隧道。
