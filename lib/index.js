import { a as signPresenceEvent, c as verifyRoomEvent, d as NETWORK_HALL_RULES, f as HALL_PROTOCOL_VERSION, i as signCheckpointEvent, l as BoundedSerialQueue, m as HALL_TOPIC, n as RoomEventLedger, o as signRoomEvent, p as HALL_SYNC_PROTOCOL, r as contentAddressProfile, s as signSyncRequest, t as MAX_SYNC_EVENTS, u as withStreamDeadline } from "./room-events-BQsHMQ7A.js";
...
function canonicalInviteBytes(payload) {
	return encoder$1.encode(JSON.stringify({
		...
		hallProtocol: payload.hallProtocol,
...
function assertDialAddress(address, expectedPeerId) {
	...
	if (!Number.isInteger(port) || !standardTls && port < 1024 || port > 65535) throw new Error("Invite address uses a disallowed port");
...
function assertPayload(value, now) {
	...
	if (candidate.version !== 2 || candidate.hallProtocol !== "0.5.1") throw new Error("HALL_PROTOCOL_MISMATCH");
	...
	if (typeof candidate.issuedAt !== "number" || typeof candidate.expiresAt !== "number" || !Number.isSafeInteger(candidate.issuedAt) || !Number.isSafeInteger(candidate.expiresAt) || candidate.issuedAt < 0 || candidate.issuedAt > now + 6e4 || candidate.expiresAt <= candidate.issuedAt) throw new Error("Invalid invite lifetime");
...
const TOPIC_SUBSCRIBER_WAIT_MS = 1e3;
...
*/
function presenceActionFor(hasPresence, lastJoinAt, now) {
...
var CarbonClubNode = class {
	...
	silentPublishDrops = 0;
	heartbeatTimer;
	checkpointTimer;
	ingestion = new BoundedSerialQueue(64);
	...
	identifiedProtocols = /* @__PURE__ */ new Map();
	...
	async start() {
		...
		try {
			...
			const node = await createLibp2p({
				...
				addresses: { listen: [...this.options.listenAddresses ?? ["/ip4/0.0.0.0/tcp/0/ws"], ...this.options.enableRelayReservations === false ? [] : bootstrapAddresses.map((address) => `${address}/p2p-circuit`)] },
				transportManager: { faultTolerance: 1 },
				transports: [webSockets(), circuitRelayTransport({
					maxReservationQueueLength: 16,
					reservationConcurrency: 2,
					reservationCompletionTimeout: 8e3
				})],
			...
			await node.handle(HALL_SYNC_PROTOCOL, async (stream, connection) => withStreamDeadline(stream, async () => {
				stream.maxReadBufferLength = 1024;
				...
				if (signedRequest.kind !== "room.sync.request" || signedRequest.origin !== remotePeerId || signedRequest.payload.targetPeerId !== node.peerId.toString()) {
					stream.abort(/* @__PURE__ */ new Error("Room sync requester is invalid"));
					return;
				}
	...
	async createInvite(now = Date.now()) {
		...
		return {
			code: encodeInvite(await signInvite(this.privateKey, {
				version: 2,
				hallProtocol: HALL_PROTOCOL_VERSION,
				roomId: "hall",
				peerId,
				addresses,
				issuedAt: now,
	...
	* An Identify advertisement is not proof that a peer behaves honestly. */
	async verifyPeerProtocol(peerId) {
		const node = this.requiredNode();
		if (node.getConnections().find((item) => item.remotePeer.toString() === peerId) === void 0) throw new Error("PEER_NOT_CONNECTED");
	...
	async joinHall(profile, now = Date.now()) {
		...
		if (!before.seats.some((seat) => seat?.participant.peerId === localPeerId) && before.localQueuePosition === void 0) this.clearLocalPresence();
		const previousPresence = this.localPresence;
		const seatJoinedAt = previousPresence?.joinedAt ?? now;
		const action = presenceActionFor(previousPresence !== void 0, this.lastJoinAt, now);
		const profileChanged = previousPresence === void 0 || !profileEquals(previousPresence.profile, profile);
		this.localPresence = {
			profile,
			joinedAt: seatJoinedAt
		};
		...
		if (snapshot.seats.some((seat) => seat?.participant.peerId === localPeerId) || snapshot.localQueuePosition !== void 0) this.ensureHeartbeat();
		else {
			this.clearLocalPresence();
	...
	async requestHistory(targetPeerId) {
		...
		try {
			...
			if (typeof decoded !== "object" || decoded === null || !("version" in decoded) || decoded.version !== 1 || !("topic" in decoded) || decoded.topic !== HALL_TOPIC || !("events" in decoded) || !Array.isArray(decoded.events) || decoded.events.length > 1300) throw new Error("Room sync snapshot is invalid");
	...
	async publishEvent(event) {
		...
		if (process.env.DSH_CARBON_CLUB_DEBUG === "1") console.warn(`[carbon-club] publish ${event.kind} mesh=${pubsub.mesh?.get(HALL_TOPIC)?.size ?? -1} topicPeers=${pubsub.topics?.get(HALL_TOPIC)?.size ?? -1} bytes=${data.byteLength}`);
		const recipients = (await pubsub.publish(HALL_TOPIC, data))?.recipients?.length ?? -1;
		if (recipients === 0) {
			this.silentPublishDrops += 1;
			if (process.env.DSH_CARBON_CLUB_DEBUG === "1") console.warn(`[carbon-club] publish reached no peer (${this.silentPublishDrops} total) kind=${event.kind} seq=${event.sequence}`);
		}
		if (process.env.DSH_CARBON_CLUB_DEBUG === "1") console.warn(`[carbon-club] published ${event.kind} seq=${event.sequence} recipients=${recipients}`);
	}
	/**
	...
	*/
	async awaitTopicSubscriber(pubsub) {
		const known = () => pubsub.topics?.get(HALL_TOPIC)?.size ?? 0;
		if (known() > 0) return;
		const node = this.node;
		if (node === void 0 || node.getConnections().length === 0) return;
