...
const HALL_TOPIC = `/dsh-human-buffer/room/hall/${HALL_PROTOCOL_VERSION}`;
const HALL_SYNC_PROTOCOL = `/dsh-human-buffer/sync/hall/${HALL_PROTOCOL_VERSION}`;
//#endregion
...
//#endregion
//#region src/network/resource-budget.ts
...
var BoundedSerialQueue = class {
	...
	enqueue(task) {
		...
		this.pendingCount += 1;
		this.tail = this.tail.then(task).catch(() => {}).finally(() => {
			this.pendingCount -= 1;
		});
		return true;
	}
...
async function withStreamDeadline(stream, action, timeoutMs = 1e4) {
	const timer = setTimeout(() => {
		stream.abort(/* @__PURE__ */ new Error("Room sync deadline exceeded"));
	}, timeoutMs);
	timer.unref();
	try {
		return await action();
...
var RoomEventLedger = class {
	admissionError(peerId, now = Date.now()) {
		const state = derive(this.events, now);
		if (participantFor(state, peerId) !== void 0) return void 0;
...
//#endregion
export { signPresenceEvent as a, verifyRoomEvent as c, NETWORK_HALL_RULES as d, HALL_PROTOCOL_VERSION as f, signCheckpointEvent as i, BoundedSerialQueue as l, HALL_TOPIC as m, RoomEventLedger as n, signRoomEvent as o, HALL_SYNC_PROTOCOL as p, contentAddressProfile as r, signSyncRequest as s, MAX_SYNC_EVENTS as t, withStreamDeadline as u };

//# sourceMappingURL=room-events-BQsHMQ7A.js.map