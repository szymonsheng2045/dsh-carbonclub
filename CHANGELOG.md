...
- Fixed a silent message loss at the start of a session. GossipSub publishes only to the peers whose topic subscription it already knows, and this node allows publishing to zero topic peers, so the first event on a fresh connection resolved successfully while nobody received it — and because the event was already in the seen cache, no retry could deliver it either. `publishEvent` now waits (bounded, one second) for a known subscriber while a connection exists, and a genuinely zero-recipient publish is counted as a debug-visible drop instead of looking like a successful send. The intermittent relay smoke failure was this race, not a flaky test.
